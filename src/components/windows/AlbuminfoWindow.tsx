...
export default function AlbuminfoWindow({ windowId }: AlbuminfoWindowProps) {
  ...
  useEffect(() => {
    if (currentIndex < albumInfo.length) {
      const timer = setTimeout(() => {
        setDisplayText(albumInfo.substring(0, currentIndex + 1));
  ...
  return (
    <div className="h-full bg-cream text-retro-black font-system">
      <div 
        className="h-full overflow-auto p-4 text-sm leading-relaxed whitespace-pre-wrap"
      >
        <div>
          {displayText}
          {currentIndex < albumInfo.length && (
